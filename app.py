...
FACILITY_DEFS = {
    ...
    "👩 Women Support": {"queries": ['node["name"~"One Stop|One-Stop|Sakhi|Shakti Sadan|Women",i]', 'way["name"~"One Stop|One-Stop|Sakhi|Shakti Sadan|Women",i]'], "color": "pink", "icon": "female"},
    "👧 Child Support": {"queries": ['node["name"~"Child|Bal|Juvenile",i]', 'way["name"~"Child|Bal|Juvenile",i]', 'node["social_facility"="child_care"]', 'way["social_facility"="child_care"]'], "color": "darkblue", "icon": "child"},
    "⚖️ Legal Support": {"queries": ['node["office"="lawyer"]', 'way["office"="lawyer"]', 'node["amenity"="courthouse"]', 'way["amenity"="courthouse"]', 'node["name"~"Legal|Court|DLSA|SLSA",i]', 'way["name"~"Legal|Court|DLSA|SLSA",i]'], "color": "black", "icon": "gavel"},
}
    ...
    for endpoint in OVERPASS_URLS:
        try:
            ...
            for el in elements:
                ...
                elif office == "lawyer" or amenity == "courthouse" or any(x in nlow for x in ["legal", "court", "dlsa", "slsa"]):
                    typ, color, icon = "Legal Support", "black", "gavel"
                    if not name: name = "Legal / Court Facility"
                elif any(x in nlow for x in ["child", "bal", "juvenile"]) or tags.get("social_facility") == "child_care":
                    typ, color, icon = "Child Support", "darkblue", "child"
                    if not name: name = "Child Support Facility"
                elif any(x in nlow for x in ["one stop", "one-stop", "sakhi", "shakti sadan", "women"]):
                    typ, color, icon = "Women Support", "pink", "female"
                    if not name: name = "Women Support Facility"
                else:
                    typ, color, icon = "Social Support", "lightpurple", "home"
                    if not name: name = "Support Facility"
