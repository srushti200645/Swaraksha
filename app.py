...
DEFAULT_LON = 73.0297
USER_AGENT = "Swaraksha-PanIndia-Safety/7.0"

# Initial Hardcoded Sample for First Launch Only
INITIAL_NAVI_MUMBAI = [
...
# Dynamic Overpass API Query for Any Location in India
def fetch_pan_india_facilities(lat: float, lon: float, radius_km: float):
    results = []
    
    # Overpass Query across 1000m * radius_km around specified search lat/lon
    try:
        ...
        (
          ...
          way["amenity"="police"](around:{int(radius_km*1000)},{lat},{lon});
          ...
          node["amenity"="social_facility"](around:{int(radius_km*1000)},{lat},{lon});
        );
        out tags center;
        """
        res = requests.post(url, data={"data": query}, headers={"User-Agent": USER_AGENT}, timeout=12)
        if res.status_code == 200:
            for elem in res.json().get("elements", []):
                ...
                if e_lat and e_lon:
                    ...
                    name = tags.get("name") or tags.get("official_name") or f"Local {kind}"
    ...
    # If Live OSM data returns empty and initial location is Navi Mumbai, use initial dataset
...
col1.metric("📍 Active City/Area", search_query.title())
...
st_folium(m, width="100%", height=500, key=f"map_{cur_lat}_{cur_lon}")
