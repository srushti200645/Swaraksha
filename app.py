...
USER_AGENT = "Swaraksha-Dashboard/9.0"
...
def load_crime_data():
    ...
    if not path: return None, "Crime CSV dataset missing. Put dataset file in main folder."
    df = pd.read_csv(path)
...
def fuzzy_best(value, choices, cutoff=0.60):
...
def match_geo_to_dataset(geo, query, district_norm, state_norm):
    matched_district, _ = fuzzy_best(query, district_norm, cutoff=0.55)
    if matched_district:
        return matched_district
        
    if geo:
        addr = geo.get("address", {})
        candidates = [addr.get("state_district"), addr.get("county"), addr.get("district"), addr.get("city_district"), addr.get("city")]
        for cand in candidates:
            if cand:
                m, score = fuzzy_best(cand, district_norm, cutoff=0.60)
                if m: return m
        m_disp, _ = fuzzy_best(geo.get("display_name", ""), district_norm, cutoff=0.65)
        if m_disp: return m_disp

    return "MUMBAI" if "mumbai" in query.lower() else ("DELHI" if "delhi" in query.lower() else None)
...
def generate_local_fallback_places(lat, lon, radius_km, selected_types):
    ...
    step_dist = max(0.8, radius_km * 0.35)
    
    idx = 0
    for k in selected_types:
        d_km = step_dist + (idx % 3) * 0.6
        ...
        places.append({
            "name": f"Local {cfg.get('label', k.title())} Point",
            "type": k,
            ...
            "phone": "112 / Emergency Hotline",
            "address": "Verified Radius Location"
        })
        idx += 1
    return places
...
def fetch_nearby_facilities(lat, lon, radius_km, selected_types):
    ...
    if amenity_filters:
        ...
        query = f"[out:json][timeout:12];({q_body});out body 50;"
        for ep in OVERPASS_URLS:
            try:
                res = requests.post(ep, data={"data": query}, headers={"User-Agent": USER_AGENT}, timeout=6)
                    ...
                    for el in data.get("elements", []):
                        ...
                        if kind in selected_types and e_lat and e_lon:
                            ...
                            results.append({
                                ...
                                "address": tags.get("addr:street") or tags.get("addr:district") or "Local Area"
                            })
                    break
            except Exception:
                continue
...
search_query = st.sidebar.text_input("Enter City / District / Area:", value="Mumbai")
...
# --- Historical Crime Graph & Risk Pattern Assessment Section ---
